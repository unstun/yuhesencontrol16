...
void waitSome(const double duration)
{
...
void sendTf(
  ...
  std::shared_ptr<tf2_ros::TransformBroadcaster> tf_broadcaster, size_t n)
{
  while (--n != 0u) {
    auto t = geometry_msgs::TransformStamped();
    t.header.frame_id = source;
    t.child_frame_id = dest;

    t.header.stamp = ros::Time::now() + ros::Duration(0.003);
