...
public:
  ...
  {
    double cx;      ///< 圆心在车体坐标系中的 x（车体前方为正），单位 m
    double cy;      ///< 圆心在车体坐标系中的 y（车体左侧为正），单位 m
    double radius;  ///< 圆半径，单位 m
  };

  // 静态 3 圆表，在 cpp 里定义初始值
  static std::array<FootprintCircle, 3> footprint_circles_;

  // 可选：供外部修改 footprint（比如从参数服务器加载）
  static void setFootprintCircles(const std::array<FootprintCircle, 3>& circles);
    // ==================== 直接使用 OccupancyGrid map 判障 ====================
  // 从 /prior_map 订阅到的静态地图
  static nav_msgs::OccupancyGrid static_map_;
  static bool has_static_map_;

  // 由外部 subscriber 调用，把 map 塞进来
  static void setStaticMap(const nav_msgs::OccupancyGrid& map)
  {
    static_map_ = map;
