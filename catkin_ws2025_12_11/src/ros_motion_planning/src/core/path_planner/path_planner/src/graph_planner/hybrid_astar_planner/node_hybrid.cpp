...
// 3 圆 footprint 默认值（单位：米，车体坐标系原点在车体中心）
std::array<NodeHybrid::FootprintCircle, 3> NodeHybrid::footprint_circles_ = {{
  { -0.30, 0.0, 0.45 },   // 后部圆
  {  0.00, 0.0, 0.45 },   // 中间圆
  {  0.30, 0.0, 0.45 }    // 前部圆
}};
...
// 静态地图（直接来自 /prior_map）
nav_msgs::OccupancyGrid NodeHybrid::static_map_;
bool NodeHybrid::has_static_map_ = false;

void NodeHybrid::setFootprintCircles(const std::array<FootprintCircle, 3>& circles)
{
  footprint_circles_ = circles;
}
...
/**
 * @brief Check if this node is valid（路线 B：优先用 static_map 判障）
 ...
 */
bool NodeHybrid::isValid(const bool& traverse_unknown,
                         const std::shared_ptr<CollisionChecker>& collision_checker)
{
  ...
  // ===== 1. 如果已经有 static_map_（来自 /prior_map），直接用它做碰撞检测 =====
  if (has_static_map_ && static_map_.info.width > 0 && static_map_.info.height > 0)
  {
    const int map_w = static_cast<int>(static_map_.info.width);
    const int map_h = static_cast<int>(static_map_.info.height);
    const double res = static_map_.info.resolution;

    // 这里假设 pose_.x(), pose_.y() 是“栅格坐标”（Hybrid A* 的搜索网格）
    // 如果是世界坐标，需要在上层保证搜索网格和 map 分辨率一致
    const double px = pose_.x();
    const double py = pose_.y();

    const double yaw = motion_table.getAngleFromBin(
        static_cast<unsigned int>(pose_.theta()));  // 弧度
    ...
    for (const auto& circle : footprint_circles_)
    {
      // 圆心从“米”转换到“栅格数”
      const double cx_local_cells = circle.cx / res;
      const double cy_local_cells = circle.cy / res;
      const double radius_cells   = circle.radius / res;

      // 旋转 + 平移到 map 的栅格坐标系
      const double center_x = px + cx_local_cells * cos_yaw - cy_local_cells * sin_yaw;
      const double center_y = py + cx_local_cells * sin_yaw + cy_local_cells * cos_yaw;

      const int cx = static_cast<int>(std::round(center_x));
      const int cy = static_cast<int>(std::round(center_y));
      const int r  = static_cast<int>(std::ceil(radius_cells));

      for (int ix = cx - r; ix <= cx + r && is_valid_; ++ix)
      {
        // 出界按碰撞处理（也可以改成根据 traverse_unknown 来决定）
        if (ix < 0 || ix >= map_w)
        {
          is_valid_ = false;
          break;
        ...
        for (int iy = cy - r; iy <= cy + r; ++iy)
        {
          ...
          const double dx = (static_cast<double>(ix) + 0.5) - center_x;
          const double dy = (static_cast<double>(iy) + 0.5) - center_y;
          if (dx * dx + dy * dy > radius_cells * radius_cells)
          {
            continue;  // 圆外，跳过
          }

          const int idx = iy * map_w + ix;
          const int8_t occ = static_map_.data[idx];

          // -1 unknown, 0 free, 100 occ（map_server 默认）
          if (occ < 0 && !traverse_unknown)
          {
            is_valid_ = false;
            break;
          }
          if (occ >= 80)  // 占据阈值，你可以按自己的 map 调整，比如 50/70/80
          {
            is_valid_ = false;
            break;
          }
  ...
  // ===== 2. 如果还没有 static_map_，退回原来的 collision_checker 逻辑（应急） =====
  ...
  // 这里可以保留你之前的 costmap 版本，或者简单用一点点逻辑，这里给个简单版：
  auto* costmap = costmap_ros->getCostmap();
  const int size_x = static_cast<int>(costmap->getSizeInCellsX());
  const int size_y = static_cast<int>(costmap->getSizeInCellsY());

  const int gx = static_cast<int>(std::round(pose_.x()));
  const int gy = static_cast<int>(std::round(pose_.y()));
  ...
  const uint64_t idx = static_cast<uint64_t>(gx) +
                       static_cast<uint64_t>(gy) * static_cast<uint64_t>(motion_table.map_width);
  is_valid_ = !collision_checker->inCollision(idx, traverse_unknown);
  cell_cost_ = collision_checker->getCost(idx);
  return is_valid_;
}
