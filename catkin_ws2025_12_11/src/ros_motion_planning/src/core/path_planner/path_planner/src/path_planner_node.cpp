...
void PathPlannerNode::initialize(std::string name)
{
  if (!initialized_)
  {
    ...
    // 订阅静态地图 /prior_map，给 Hybrid A* 的三圆碰撞检测用
    static_map_sub_ = private_nh.subscribe<nav_msgs::OccupancyGrid>(
        "/prior_map",                     // 话题名
        1,                                // 队列长度
        &PathPlannerNode::staticMapCallback,  // 成员函数回调
        this);                            // this 指针
...
/**
 * @brief OccupancyGrid callback for Hybrid A*
 */
void PathPlannerNode::staticMapCallback(const nav_msgs::OccupancyGrid::ConstPtr& msg)
{
  // 把 /prior_map 塞进 Hybrid A* 的静态地图里
