 ...
 * @author: Yang Haodong
 * @date: 2024-01-31
 * @version: 1.2 (Modified for Direct Reverse Output)
 *
...
bool MPCController::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
  ...
  // Current Pose angle (robot heading)
  ...
  // Calculate target relative to robot
  ...
  // To directly output the reverse/negative turning angle, we use the "Mirror" strategy.
  // 1. If reversing, we pretend the robot is facing the opposite way (theta + PI).
  // 2. We set the reference velocity to Positive.
  // 3. We let MPC solve as if driving FORWARD.
  // 4. Finally, we negate the output commands.

  double mpc_theta_current = theta;
  double mpc_v_ref = v_abs; // Always positive for optimization stability

  if (direction < 0.0)
  {
      // Flip the robot's orientation logic: "My back is now my front"
      mpc_theta_current = regularizeAngle(theta + M_PI);
      
      // Note: We do NOT modify theta_trj here with + PI. 
      // We calculate error relative to the FLIPPED robot heading.
  }
  // ---------------- MODIFICATION END ----------------
  ...
  {
    // Use the potentially flipped theta for state calculation
    Eigen::Vector3d s(robot_pose_map.pose.position.x, robot_pose_map.pose.position.y, mpc_theta_current);  // current state
    Eigen::Vector3d s_d(lookahead_pt.point.x, lookahead_pt.point.y, theta_trj);                            // desired state

    // Reference control
    // Because we are treating this as a Forward problem (even if reversing), we use positive velocity.
    // The sign of Kappa determines the turn direction relative to movement.
    // If direction < 0, we are "mirroring", so we trust the MPC to find the turn for the mirrored setup.
    double omega_ref = mpc_v_ref * kappa; 
    Eigen::Vector2d u_r(mpc_v_ref, omega_ref);

    // Solve MPC (Always solving a "Forward" problem now)
    Eigen::Vector2d u = _mpcControl(s, s_d, u_r, du_p_);

    // ---------------- MODIFICATION START ----------------
    // Post-Process: Flip the signs if we were reversing
    double final_v = u[0];
    double final_w = u[1];

    if (direction < 0.0)
    {
        final_v = -final_v; // Linear: Forward -> Backward
        final_w = -final_w; // Angular: Left -> Right (Inverse Steering)
    }
    // ---------------- MODIFICATION END ----------------

    double u_v = linearRegularization(base_odom, final_v);
    double u_w = angularRegularization(base_odom, final_w);

    // Update previous error (be careful to use the raw MPC output u, or the constrained u_v/u_w?)
    // Typically we track the diff between Actual Command and Reference.
    // Since we flipped the command, we should compare against the flipped reference?
    // To be safe and simple, we store the error as calculated by the MPC *before* the sign flip, 
    // because the next iteration will also perform the flip if needed.
    du_p_ = Eigen::Vector2d(u[0] - u_r[0], regularizeAngle(u[1] - u_r[1]));
